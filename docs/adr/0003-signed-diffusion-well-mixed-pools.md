...
- `cap = kCap × bodyArea` becomes a maximum internal *concentration* — a physical constant rather than an arbitrary bucket size — and only binds when the world is richer than `kCap`.
