...
A single global PRNG, as originally drafted. Deterministic, but fragile: draw order depends on population size and array position, so changing brownian motion to consume three random numbers instead of two shifts _every mutation in the run_ and invalidates every seed saved as an interesting run. On a project whose point is finding and replaying interesting runs, that is the wrong fragility.
