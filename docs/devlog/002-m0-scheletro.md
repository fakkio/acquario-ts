---
title: "M0: lo scheletro"
description: "La prima milestone di AcquarioTS: loop deterministico, pan e zoom, e la scoperta di cosa, nel lavoro con un agente, è davvero difficile lasciare andare."
date: 2026-08-31
authors:
  - Fabio Lazzaroni
  - Claude
...
Inizio l'implementazione della M0 di AcquarioTS, la milestone che porterà a un loop di aggiornamento e alla gestione di pan e zoom, sia via tastiera e mouse sia via touch. L'obiettivo è far programmare tutto al mio fidato agente: io faccio solo da architetto, prendendo le decisioni importanti sul perché, il cosa e il come.

Già, il come. Ma non mi interessa imporre all'agente dettagli implementativi come la forma o lo stile di programmazione — in questo gli agenti ormai sono fortissimi. Fatico ancora a fidarmi però: a volte ha ancora bisogno di una correzione, a volte è lui a convincere me.
...
M0 è la prima milestone: prima che ci sia comportamento, deve esserci uno scheletro che si regge da solo. Dentro c'è un accumulatore a passo fisso — la parte del loop che decide quanti `tick` di simulazione far girare a ogni frame, indipendentemente da quanto sia irregolare il framerate del browser — e c'è `mountCanvas`, la funzione che prende un elemento del DOM e ci disegna dentro la griglia zoomabile.

Il modo per sapere se un accumulatore a passo fisso funziona davvero è verificare che due mondi partiti dallo stesso seed arrivino, tick dopo tick, allo stesso stato. Per questo esiste `hashState` — e l'ho capito male anche io, rileggendolo a distanza di settimane. Non è quello che rende il mondo deterministico: quel lavoro lo fa `createWorld`, con il seed e gli stream di PRNG assegnati a ogni organismo. `hashState` è la sonda che confronta due mondi e dice se sono ancora uguali. Il test in `world.test.ts` per ora la esercita solo sulla meccanica sintetica del `World` — accumulator, PRNG — perché non esiste ancora niente che possa divergere in modo interessante.
...
È un fatto piccolo, ma vale la pena fermarcisi. Sui tre bottoni di prima, il problema l'ho visto io. Qui se n'è accorto lui, prima che arrivassi a guardare quel codice. Sono due fatti diversi, anche se da fuori il risultato — codice migliorato — sembra identico.

Il "come" che decido io e il "come" che non correggo in review non sono lo stesso "come". Uno è il come di progetto: canvas, TypeScript, Prettier, come si fanno i commit, i branch, quali skill usare. Resta mio al 100%. `bracketSpacing` — niente spazi dentro le parentesi graffe, lo stile che uso in tutti i miei progetti — è un esempio di questo tipo, e non l'ho messo nella config fin dall'inizio: ho lasciato girare Prettier di default finché non mi sono imbattuto per caso in un file pieno di spazi che non mi piaceva. A quel punto ho disattivato l'opzione e riformattato tutto il repo, in un commit separato.
