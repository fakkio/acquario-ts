...
The agent drafts the article end to end. Fabio's hands are on the **pile**: `/writing-fragments` is where his material enters, and the article is only as good as what that stage pulled out of him. Inside the writing skills his input is the decisions they ask for — grounding, which block comes next, what to cut.

At the end of a milestone, on the milestone branch and **before the merge to `develop`**:

1. **Draft from the pile** — `/writing-beats` or `/writing-shape`, chosen as below.
2. **Editing pass over the draft** — `/writing-shape`, with the draft itself as the pile.
3. **Move the result over `docs/devlog/00N-<slug>.md`** and drop the intermediate files. Both skills treat their input as read-only and write to a new file, so the final article has to be put in place by hand.
4. Commit with `📝(devlog):`, then merge.

Until 2026-09-13 this ran as a **hands-first protocol**: Fabio wrote a growing share of each article by hand and blind, before any generated prose, and the draft was committed separately so the editing pass landed as a diff over his own paragraphs. He ended that deliberately — the writing-practice goal behind it is closed. Articles 001 and 002 still contain paragraphs written that way.
...
For step 2 use **shape** regardless: an editing pass is an argument about what each block is doing.
...
`writing-fragments`, `writing-beats` and `writing-shape` are **not referenced by the `ask-matt` map**, so they sit outside the maintained flow. `/edit-article`, which this pipeline once ended on, vanished in an upstream update with no warning, and these can too.
